<?php
class NametoGender extends Task{
	public $metaphone_function;
	public $DEBUG;
	public $MATCH_LIST;
	public $DEBUG_MSG;
	public $Females;
	...
	public function __construct(){
		...
		if(is_callable($this->double_metaphone())){
	...
	public function gender_init($females_ref=null, $males_ref=null) {
	...
	public function gender($name, $looseness=1) {
	    $gender = null;
	    ...
	    for ($i = 0; $i < $looseness; $i++) {
	        ...
	        if ($this->DEBUG)
	            $this->DEBUG_MSG .= "\t{$this->MATCH_LIST[$i]}...\n";

	        $gender = $this->MATCH_LIST[$i]($name);

	        if ($this->DEBUG && isset($gender))
	            $this->DEBUG_MSG .= "\t==> HIT ($gender)\n";
	...
	public function either_weight($name) {
	    $gender = null;
	    ...
	    if ($this->DEBUG && isset($gender))
	        $this->DEBUG_MSG .= "\tF: $female_hit, M: $male_hit\n";

	    return $gender;
	}

	public function one_only_metaphone($name) {
	    $gender = null;

	    // Match one list only, use DoubleMetaphone

	    $meta_name = $this->metaphone_function($name);
	    $metaphone_hit = '';
	    ...
	    foreach ($this->Females as $list_name => $weight)
	    {
	        ...
	        $meta_list_name = $this->metaphone_function($list_name);

	        if ($meta_name === $meta_list_name) {
	            $female_hit = $weight;

	            if ($this->DEBUG)
	                $this->DEBUG_MSG .= sprintf("\tF: %s => %s => %s: %f\n",
	...
	public function either_weight_metaphone($name) {
	    $gender = null;
